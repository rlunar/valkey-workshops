...
parser.add_argument('--ssl', type=lambda x: (str(x).lower() == 'true'), default=False, help='True/False Enable/Disable Valkey TLS default is False')
...
READ_QUERY = text("select p.firstname, p.lastname, count(*) \
                from flughafendb.passenger p, flughafendb.booking b \
                where p.passenger_id = :passenger \
                and p.passenger_id = b.passenger_id \
                group by p.firstname, p.lastname")

WRITE_QUERY = text("INSERT into flughafendb.booking (flight_id, passenger_id, price, seat) \
...
try:
  ...
  redis_read  = Redis(host=params['ec_redis_host'], port=params['ec_redis_port'], ssl=args.ssl, decode_responses=False, socket_connect_timeout = 5)
  # To force all connections to the primary
  # redis_read = redis_write
  print("Connected to Valkey")

except Exception as e:
  print("SQLAlchemy create engine connection or Valkey exception occurred")
  print(e)
  exit(1)
